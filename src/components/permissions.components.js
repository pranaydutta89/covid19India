...
const permissions = {
    locations: {
        name: 'Geolocation',
        description: 'With this we will try to guess your current location and give details according to your location'
    },
    notification: {
        name: 'Push Notification',
        description: 'With this we will send realtime information of your current location'
    },
}
...
export default class PermissionsComponent extends Component {
    ...
    async startTimer() {
        ...
        while (true) {
            await new Promise((res) => {
                ...
                setTimeout(async () => {
                    await utilsService.stateSync(this.setState.bind(this), { timerValue: timerValue - 10 });
                    res()
                }, 1000);
            });
            const { timerValue } = this.state;
    ...
    async startPermissionsAsking() {
        try {
            ...
            await utilsService.stateSync(this.setState.bind(this), { timerValue: 100 });
        }

        try {
            await utilsService.stateSync(this.setState.bind(this), { currentPermission: 'notification' });
            await this.startTimer();
