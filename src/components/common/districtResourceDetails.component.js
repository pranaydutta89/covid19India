...
import { Card, CardContent, Typography, CardActions, Link, LinearProgress } from '@material-ui/core';
...
export default class DistrictResourceDetails extends Component {
    ...
    async componentDidMount() {
        ...
        this.setState({ resourceDetails: await covidDataService.getDistrictResources(districtName) });
    }
    ...
    render() {
        ...
        return (
            <>
                {!resourceDetails ? <LinearProgress /> :
                    <>
                        {resourceDetails.map(({ id,
                            category,
                            source,
                            notes,
                            name,
