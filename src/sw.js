...
class PoolDataAndProcess {
  ...
  async checkIndiaDiff() {
    ...
    if (previous && latest) {
      const {
        cases: { total: oldTotal },
      } = previous;
      const {
        cases: { total: newTotal },
      } = latest;
      let msg = '';
      ...
      if (msg) {
        await this.showNotification('India, new summary', msg);
      }
    }
  }
  async checkLocationDiff() {
    ...
    if (previous && latest && locationData) {
      ...
      if (stateMsg) {
        ...
        await this.showNotification(`state ${stateLongName}`, stateMsg);
      }
      ...
      if (cityMsg) {
        ...
        await this.showNotification(`city ${cityLongName}`, cityMsg);
      }
    }
  }
...
self.addEventListener('activate', function (event) {
  event.waitUntil(new PoolDataAndProcess().startPooling());
});
