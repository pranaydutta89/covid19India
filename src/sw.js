...
class PoolDataAndProcess {
    ...
    async checkLocationDiff(oldData, newData) {
        ...
        if (locationData) {
            ...
            if (stateMsg) {
                const { state: { longName: stateLongName } } = locationData;
                this.showNotification(`Covid Alert for state ${stateLongName}`, stateMsg)
            }

            const cityMsg = this.cityDiffMessage(oldData, newData, locationData);
            if (cityMsg) {
                const { city: { longName: cityLongName } } = locationData;
