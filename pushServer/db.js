...
class DB {
  constructor() {
    this.dbFileName =
      process.env.NODE_ENV === 'production' ? 'db_prod.json' : 'db_dev.json';
  }
  ...
  async stateData() {
    ...
    for (let i = 1; i < statewise.length; i++) {
      state_stats.push(
        utils.getSelectedAttirbutes(constants.stateKeys, statewise[i])
      );
    }
    return state_stats;
  }
  async districtData() {
    ...
    stateWithDistrict.forEach(({ state, districtData }) => {
      districtData.forEach(
        ({ active, confirmed, deceased: deaths, recovered, district }) => {
          district_stats.push({
            active,
            confirmed,
            deaths,
